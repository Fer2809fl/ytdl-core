...
exports.playError = player_response => {
  ...
  // Detectar bloqueo anti-bot en mensajes de playability
  const reason = (playability.reason || playability.messages?.[0] || "").toLowerCase();
  if (playability.status === "LOGIN_REQUIRED") {
    if (
      ...
      const err = new LoginRequiredError(
        "YouTube bloqueo esta IP/sesion por deteccion de bot. Usa cookies frescas de la MISMA IP, PO Token, o espera a que expire el rate limit."
      );
      err.isBotBlock = true;
      return err;
    }
    return new LoginRequiredError(playability.reason || playability.messages?.[0]);
  }
  if (playability.status === "ERROR") {
    return new UnavailableError(playability.reason || playability.messages?.[0]);
  }
  if (playability.status === "LIVE_STREAM_OFFLINE") {
    return new LiveStreamOfflineError(playability.reason || "The live stream is offline.");
  }
  if (playability.status === "UNPLAYABLE") {
    return new UnavailableError(playability.reason || "This video is unavailable.");
  }
  return null;
};
...
exports.request = async (url, options = {}) => {
  ...
  // Detectar bloqueo anti-bot por redirect a google.com/sorry
  if (code === "303" || code === "403") {
    const location = req.headers.location || "";
    if (location.includes("google.com/sorry") || location.includes("consent.youtube.com")) {
      throw new LoginRequiredError(
        "YouTube bloqueo esta IP/sesion por deteccion de bot (CAPTCHA/verificacion). Usa cookies frescas de la MISMA IP, PO Token, o espera a que expire el rate limit."
      );
    }
  }
  ...
  throw new StatusCodeError(`Status code: ${code}`, req.statusCode);
};
...
exports.checkForUpdates = () => {
  if (
    ...
    return exports
      .request("https://api.github.com/repos/Fer2809fl/ytdl-core/contents/package.json", {
        requestOptions: {
          headers: {
            "User-Agent":
      ...
      .then(
        response => {
          ...
          if (pkgFile.version !== pkg.version && updateWarnTimes++ < 5) {
            ...
            console.warn(
              '\x1b[33mWARNING:\x1B[0m @fer2809fl/ytdl-core is out of date! Update with "npm install @fer2809fl/ytdl-core@latest".',
            );
          }
        },
        err => {
...
const getRandomIPv6 = ip => {
  if (!isIPv6(ip)) {
    throw new YtdlError("Invalid IPv6 format");
  }
  ...
  if (isNaN(mask) || mask > 128 || mask < 1) {
    throw new YtdlError("Invalid IPv6 subnet mask (must be between 1 and 128)");
  }
      ...
      if (oldCookieWarning) {
        ...
        console.warn(
          "\x1b[33mWARNING:\x1B[0m Using old cookie format, " +
            "please use the new one instead. (https://github.com/Fer2809fl/ytdl-core#cookies-support)",
        );
      }
    }
    if (options.requestOptions.dispatcher && oldDispatcherWarning) {
      ...
      console.warn(
        "\x1b[33mWARNING:\x1B[0m Your dispatcher is overridden by `ytdl.Agent`. " +
          ...
          "(https://github.com/Fer2809fl/ytdl-core#how-to-implement-ytdlagent-with-your-own-dispatcher)",
      );
    }
  ...
  if (oldLocalAddressWarning) {
    ...
    console.warn(
      "\x1b[33mWARNING:\x1B[0m Using old localAddress option, " +
        "please add it to the agent options instead. (https://github.com/Fer2809fl/ytdl-core#ip-rotation)",
    );
  }
};
    ...
    if (oldIpRotationsWarning) {
      ...
      console.warn(
        "\x1b[33mWARNING:\x1B[0m IPv6Block option is deprecated, " +
          "please create your own ip rotation instead. (https://github.com/Fer2809fl/ytdl-core#ip-rotation)",
      );
    }
  }
};
