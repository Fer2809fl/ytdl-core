...
const { NoFormatsError, ParsingError, LoginRequiredError } = require("./errors");
...
exports.getBasicInfo = async (id, options) => {
  ...
  // Intentar watch.html primero (para likes, related, etc.)
  let info;
  try {
    info = await retryFunc(getWatchHTMLPage, [id, options], retryOptions);
  } catch (err) {
    // Si es bloqueo anti-bot, relanzar
    if (err instanceof LoginRequiredError) throw err;
    // Si no, empezar con objeto vacio
    info = { page: "watch" };
  }

  // FALLBACK CRITICO: si watch.html no entrego player_response, ir directo a la API WEB
  if (!info.player_response) {
    try {
      info.player_response = await fetchWebPlayer(id, options);
    } catch (apiErr) {
      throw new ParsingError(
        "Error when parsing watch.html and player API failed: " + apiErr.message
      );
    }
  }

  // FALLBACK: html5player desde embed si no lo tenemos
  if (!info.html5player) {
    try {
      const embedBody = await getEmbedPageBody(id, options);
      info.html5player = getHTML5player(embedBody);
    } catch (_) {
      // ignore
    }
  }
  ...
  Object.assign(info, {
    related_videos: info.response ? extras.getRelatedVideos(info) : [],
  });

  // Add additional properties to info.
  const media = info.response ? extras.getMedia(info) : {};
  const additional = {
    author: info.response ? extras.getAuthor(info) : {},
    media,
    likes: info.response ? extras.getLikes(info) : null,
    age_restricted: !!(
    ...
    storyboards: info.player_response ? extras.getStoryboards(info) : [],
    chapters: info.response ? extras.getChapters(info) : [],
  };
...
const parseJSON = (source, varName, json) => {
  if (!json || typeof json === "object") {
    ...
    try {
      ...
      throw new ParsingError(`Error parsing ${varName} in ${source}: ${err.message}`);
    }
  }
};
...
const findJSON = (source, varName, body, left, right, prependJSON) => {
  ...
  if (!jsonStr) {
    throw new ParsingError(`Could not find ${varName} in ${source}`);
  }
...
const getWatchHTMLPage = async (id, options) => {
  ...
  // Detectar CAPTCHA / bloqueo anti-bot
  if (
    ...
    throw new LoginRequiredError(
      "YouTube bloqueo esta IP/sesion (CAPTCHA/verificacion). Usa cookies frescas de la MISMA IP, PO Token, o espera."
    );
  }

  // Si el HTML no tiene los scripts de ytInitial, devolver vacio para que el fallback API entre
  if (body.length < 1000 || !body.includes("ytInitial")) {
    return info;
  }
  ...
  try {
    try {
      info.player_response =
        ...
        utils.tryParseBetween(body, 'window["ytInitialPlayerResponse"] = ', "}};", "", "}}") ||
        utils.tryParseBetween(body, 'window["ytInitialPlayerResponse"] = ', ";</script>") ||
        utils.tryParseBetween(body, "window.ytInitialPlayerResponse = ", "}};", "", "}}") ||
        utils.tryParseBetween(body, "window.ytInitialPlayerResponse = ", ";</script>") ||
    ...
    // Si falla el parsing, no es fatal, devolver lo que tengamos (aunque sea vacio)
  }
  return info;
};
...
exports.getInfo = async (id, options) => {
  ...
  if (!info.html5player) {
    throw new ParsingError("Unable to find html5player file");
  }
  ...
  if (formatPromises.length === 0) {
    throw new NoFormatsError("Failed to find any playable formats");
  }
  ...
  if (info.formats.length === 0) {
    throw new NoFormatsError("No playable formats found");
  }
...
const WEB_CONTEXT = {
  client: {
    clientName: "WEB",
    clientVersion: "2.20250820.01.00",
    ...LOCALE,
  },
};
...
const TVHTML5_CONTEXT = {
  client: {
    clientName: "TVHTML5",
    clientVersion: "7.20250820.13.00",
    ...LOCALE,
  },
};

const fetchWebPlayer = async (videoId, options) => {
  const payload = {
    context: WEB_CONTEXT,
    videoId,
    ...CHECK_FLAGS,
...
const playerAPI = async (videoId, payload, options) => {
  ...
  if (!response.videoDetails || videoId !== response.videoDetails.videoId) {
    const err = new ParsingError("Malformed response from YouTube");
...
const IOS_CLIENT_VERSION = "20.15.4",
  IOS_DEVICE_MODEL = "iPhone17,2",
  IOS_USER_AGENT_VERSION = "18_1_1",
  IOS_OS_VERSION = "18.1.1.22B91";
...
const ANDROID_CLIENT_VERSION = "20.14.38",
