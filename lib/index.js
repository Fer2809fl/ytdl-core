...
const downloadFromInfoCallback = (stream, info, options) => {
  ...
  if (!info.formats.length) {
    stream.emit("error", new errors.NoFormatsError("This video is unavailable"));
    return;
  }
...
ytdl.downloadFromInfo = (info, options) => {
  const stream = createStream(options);
  if (!info.full) {
    throw new errors.YtdlError("Cannot use `ytdl.downloadFromInfo()` when called with info from `ytdl.getBasicInfo()`");
  }
  setImmediate(() => {
    downloadFromInfoCallback(stream, info, options);
  });
...
/**
 * Busca en YouTube por texto y descarga el primer resultado.
 * Combina `ytdl.searchOne()` + `ytdl.getInfo()` + descarga en un solo stream,
 * igual que llamar a `ytdl(url)` pero a partir de una búsqueda en vez de una URL.
 *
 * Emite el evento `searchResult` con el video elegido antes de empezar a
 * descargar, además de los eventos normales (`info`, `progress`, `error`, etc).
 *
 * @param {string} query
 * @param {!Object} options
 * @returns {ReadableStream}
...
ytdl.downloadFromQuery = (query, options) => {
  const stream = createStream(options);
  search.searchOne(query).then(
    video => {
      if (stream.destroyed) return;
      stream.emit("searchResult", video);
      ytdl.getInfo(video.url, options).then(
